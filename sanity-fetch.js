...
async function initArchive() {
    ...
    // Fetch projects with their resolved references
...
function renderGrid(data) {
    ...
    grid.innerHTML = '';
    counter.innerText = data.length.toString().padStart(3, '0');
    ...
    if (currentView === 'list') {
        ...
        data.forEach(p => {
            ...
            if (p.thumbnail?.asset?._ref) {
                ...
                thumbUrl = `https://cdn.sanity.io/images/${PROJECT_ID}/${DATASET}/${ref.replace('image-', '').replace('-jpg', '.jpg').replace('-png', '.png')}?w=600&fit=crop`;
            }
